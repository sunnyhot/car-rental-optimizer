...
struct PersistedZucheCookie: Codable, Equatable {
    let name: String
    let value: String
    let domain: String
    let path: String
    ...
    init?(cookie: HTTPCookie, now: Date = Date()) {
        guard ZucheCookieVault.isZucheCookie(cookie) else { return nil }
...
enum ZucheCookieVault {
    private static let directoryName = "CarRentalOptimizer"
    private static let fileName = "zuche-session-cookies.json"
    ...
    static func isZucheCookie(_ cookie: HTTPCookie) -> Bool {
        let domain = cookie.domain.trimmingCharacters(in: CharacterSet(charactersIn: ".")).lowercased()
        return domain == "zuche.com" || domain.hasSuffix(".zuche.com")
    }

    static func persistableCookies(from cookies: [HTTPCookie], now: Date = Date()) -> [PersistedZucheCookie] {
        cookies
            .compactMap { PersistedZucheCookie(cookie: $0, now: now) }
            .sorted { lhs, rhs in
    ...
    static func restoredCookies(from cookies: [PersistedZucheCookie], now: Date = Date()) -> [HTTPCookie] {
    ...
    static func restore(into store: WKHTTPCookieStore, fileURL: URL = defaultCookieFileURL) async {
        ...
        guard let persisted = try? JSONDecoder().decode([PersistedZucheCookie].self, from: data) else { return }
    ...
    static func save(from store: WKHTTPCookieStore, fileURL: URL = defaultCookieFileURL) async {
        ...
        do {
            ...
            assertionFailure("Failed to persist CAR Inc cookies: \(error.localizedDescription)")
        }
    }
}
